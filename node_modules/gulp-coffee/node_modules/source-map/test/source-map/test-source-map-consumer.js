...
define(function (require, exports, module) {
  ...
  exports['test that the `sources` field has the original sources'] = function (assert, util) {
    ...
    sources = map.sources;
    assert.equal(sources[0], '/the/root/one.es');
    assert.equal(sources[1], '/the/root/two.es');
  ...
  exports['test that the source root is reflected in a mapping\'s source field'] = function (assert, util) {
    ...
    assert.equal(mapping.source, '/the/root/two.es');

    mapping = map.originalPositionFor({
      line: 1,
      column: 1
    });
    assert.equal(mapping.source, '/the/root/one.es');
  ...
  exports['test mapping tokens back exactly'] = function (assert, util) {
    ...
    util.assertMapping(1, 1, '/the/root/one.es', 1, 1, null, map, assert);
    util.assertMapping(1, 5, '/the/root/one.es', 1, 5, null, map, assert);
    util.assertMapping(1, 9, '/the/root/one.es', 1, 11, null, map, assert);
    util.assertMapping(1, 18, '/the/root/one.es', 1, 21, 'bar', map, assert);
    util.assertMapping(1, 21, '/the/root/one.es', 2, 3, null, map, assert);
    util.assertMapping(1, 28, '/the/root/one.es', 2, 10, 'baz', map, assert);
    util.assertMapping(1, 32, '/the/root/one.es', 2, 14, 'bar', map, assert);

    util.assertMapping(2, 1, '/the/root/two.es', 1, 1, null, map, assert);
    util.assertMapping(2, 5, '/the/root/two.es', 1, 5, null, map, assert);
    util.assertMapping(2, 9, '/the/root/two.es', 1, 11, null, map, assert);
    util.assertMapping(2, 18, '/the/root/two.es', 1, 21, 'n', map, assert);
    util.assertMapping(2, 21, '/the/root/two.es', 2, 3, null, map, assert);
    util.assertMapping(2, 28, '/the/root/two.es', 2, 10, 'n', map, assert);
  };
  ...
  exports['test mapping tokens fuzzy'] = function (assert, util) {
    ...
    util.assertMapping(1, 20, '/the/root/one.es', 1, 21, 'bar', map, assert, true);
    util.assertMapping(1, 30, '/the/root/one.es', 2, 10, 'baz', map, assert, true);
    util.assertMapping(2, 12, '/the/root/two.es', 1, 11, null, map, assert, true);

    // Finding generated positions
    util.assertMapping(1, 18, '/the/root/one.es', 1, 22, 'bar', map, assert, null, true);
    util.assertMapping(1, 28, '/the/root/one.es', 2, 13, 'baz', map, assert, null, true);
    util.assertMapping(2, 9, '/the/root/two.es', 1, 16, null, map, assert, null, true);
  };
  ...
  exports['test eachMapping'] = function (assert, util) {
    ...
    map.eachMapping(function (mapping) {
      ...
      assert.ok(mapping.source === '/the/root/one.es' || mapping.source === '/the/root/two.es');
